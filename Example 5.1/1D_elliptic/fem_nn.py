...
forward_data=np.load("E:/project/fem_nn/fem_nn_1D_elliptic/forward_data.npy")
forward_mesh=np.load("E:/project/fem_nn/fem_nn_1D_elliptic/forward_mesh.npy") #m.p on fine mesh
...
class Net_fem(keras.Model):# type: ignore
    ...
    def train_step(self, data):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # mx0=tf.linspace(0.,1.,3201)
            # mx=tf.reshape(mx0,(-1,1))
            # conductivity_fine=self(mx)
            # loss1=reg_para*error_dU(conductivity_fine)
...
# udag-udag=1e-7
# loss with exact q=2e-7, noise=1e-3
...
ax.plot(loss_value2)
plt.show()
# np.save("E:/project/fem_nn/fem_nn_1D_elliptic/loss_1D_2_16_5e-2.npy",loss_value2)
error_value=error_value[1:np.size(error_value,0)]
fig, ax = subplots()
ax.plot(error_value)
plt.show()
# np.save("E:/project/fem_nn/fem_nn_1D_elliptic/error_1D_2_16_5e-2.npy",error_value)
